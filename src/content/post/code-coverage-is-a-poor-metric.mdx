...
I'm not sure where the blame lies, typescript most likely, but I will never be able to cover that line because its not valid scenario given the short-circuit. Yet typescript won't narrow to `string` thus the safety valve of the nullish coelecence, and an uncovered line by extension.
